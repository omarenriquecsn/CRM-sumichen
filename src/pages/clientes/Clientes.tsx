...
export const Clientes: React.FC = () => {
  ...
  const { clientes: clientesDB } = supabase.useClientes();
  ...
  const handleCreateCliente = async (user: ClienteFormData) => {
    try {
      ...
      const newClientes = await supabase.crearCliente(user, currentUser);

      setClientes([...newClientes, ...clientesDB]);
                  ...
                  <tr key={cliente.id} className="hover:bg-gray-50">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center space-x-1 text-sm text-gray-500">
                        ...
                        <span>{cliente.ultima_actividad.toLocaleString()}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                      <button
