...
export const Pipeline: React.FC = () => {
  ...
  const { useOportunidades} = useSupabase()
  ...
  const oportunidadesPorEtapa = {
    inicial:[useOportunidades().oportunidades.filter(o => o.etapa === 'inicial')],
    calificado:[useOportunidades().oportunidades.filter(o => o.etapa === 'calificado')],
    propuesta:[useOportunidades().oportunidades.filter(o => o.etapa === 'propuesta')],
    negociacion:[useOportunidades().oportunidades.filter(o => o.etapa === 'negociacion')],
    cerrado:[useOportunidades().oportunidades.filter(o => o.etapa === 'cerrado')]
  }
  ...
  const calcularTotalEtapa = (etapaId: string) => {
     console.log(oportunidadesPorEtapa)
    return oportunidades[etapaId as keyof typeof oportunidades]?.reduce(
      (sum, oportunidad) => sum + oportunidad.valor, 0
    ) || 0;
  };
    ...
    >
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {etapas.map((etapa) => (
            <div key={etapa.id} className="bg-white rounded-lg p-4 border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-medium text-gray-900">{etapa.titulo}</h3>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getColorClasses(etapa.color)}`}>
                  {oportunidades[etapa.id as keyof typeof oportunidades]?.length || 0}
                </span>
              </div>
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
            {etapas.map((etapa) => (
              <div key={etapa.id} className="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <Droppable droppableId={etapa.id}>
                  {(provided, snapshot) => (
                    ...
                    >
                      {oportunidades[etapa.id as keyof typeof oportunidades]?.map((oportunidad, index) => (
                        <Draggable
                          key={oportunidad.id}
                            ...
                            >
                              <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className={`text-xs font-medium ${getProbabilityColor(oportunidad.probabilidad)}`}>
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-1.5">
                                  <div
                                    className={`h-1.5 rounded-full ${
                                      oportunidad.probabilidad >= 80 ? 'bg-green-500' :
                                      oportunidad.probabilidad >= 60 ? 'bg-yellow-500' :
                                      oportunidad.probabilidad >= 40 ? 'bg-orange-500' :
                                      'bg-red-500'
                                    }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">
                ${Object.values(oportunidades).flat().reduce((sum, op) => sum + op.valor, 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-600">Valor Total Pipeline</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-orange-600">
                ${Object.values(oportunidades).flat().reduce((sum, op) => sum + (op.valor * op.probabilidad / 100), 0).toLocaleString()}
              </p>
              <p className="text-sm text-gray-600">Valor Ponderado</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {Math.round(Object.values(oportunidades).flat().reduce((sum, op) => sum + op.probabilidad, 0) / Object.values(oportunidades).flat().length)}%
              </p>
